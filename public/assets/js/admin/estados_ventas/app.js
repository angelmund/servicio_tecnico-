        ...
        fetch(url, {
            ...
            .then((data) => {
                if (data.message === "Error de validación") {
                    ...
                    agregarRegistrotablaEstadosVenta(data);
                    resolve(data);
                } else if (data.Noti == 2) {
                    alertarExito(data.message);
                    actualizarTablaEstadosVenta(data);
                    resolve(data);
                } else {
...
function agregarRegistrotablaEstadosVenta(data) {
    const columnConfig = [
        ...
        { field: "activo", type: "badge", trueValue: true },
        {
            type: "actions",
            routes: {
                create: routes.estadosVentaCreate,
                edit: routes.estadosVentaEdit,
                cambiarEstado : routes.estadosVentacambiarEstado,
            },
        },
    ];
...
function actualizarTablaEstadosVenta(data) {
    const columnConfig = [
        { field: "nombre", type: "text" },
        { field: "descripcion", type: "text" },
        { field: "activo", type: "badge", trueValue: true },
    ...
    // Asegurarse de que estamos pasando el objeto correcto a actualizarTabla
    const estadoVenta = data.estado_venta || data;

    // Asegurarse de que 'activo' sea un booleano
    if (typeof estadoVenta.activo === 'number') {
        estadoVenta.activo = Boolean(estadoVenta.activo);
    }

    actualizarTabla(estadoVenta, "datatables", columnConfig);
}

document.addEventListener("DOMContentLoaded", function () {
    if (!$.fn.DataTable.isDataTable("#datatables")) {
        tablaEstadosVenta = initDataTable("datatables");
    } else {
        tablaEstadosVenta = $("#datatables").DataTable();
    }
    ...
    function cambiarEstado(id, url, action) {
        mostrarConfirmacion(
            `¿Deseas ${action} el estado de venta?`,
            () => {
                return new Promise((resolve, reject) => {
                    fetch(url, {
                        method: "POST",
