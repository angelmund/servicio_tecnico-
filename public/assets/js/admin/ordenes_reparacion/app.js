        ...
        fetch(url, {
            ...
            .then((data) => {
                if (data.message === "Error de validación") {
                    ...
                    agregarRegistroTablaOrdenReparacion(data);
                    resolve(data);
                } else if (data.Noti == 2) {
                    alertarExito(data.message);
                    actualizarTablaOrdenReparacion(data);
                    resolve(data);
                } else {
...
function agregarRegistroTablaOrdenReparacion(data) {
    const columnConfig = [
        ...
        { field: "activo", type: "badge", trueValue: true },
        {
            type: "actions",
            routes: {
                //create: routes.estadosReparacionCreate,
                edit: routes.estadosReparacionEdit,
                //cambiarEstado : routes.estadosReparacioncambiarEstado,
            },
        },
    ];
...
function actualizarTablaOrdenReparacion(data) {
    const columnConfig = [
        { field: "nombre", type: "text" },
        { field: "descripcion", type: "text" },
        { field: "activo", type: "badge", trueValue: true },
...
document.addEventListener("DOMContentLoaded", function () {
    if (!$.fn.DataTable.isDataTable("#datatables")) {
        tablaOrdenReparacion = initDataTable("datatables");
    } else {
        tablaOrdenReparacion = $("#datatables").DataTable();
    }

    ["btn-guardar", "btn-actualizar", "btnL_guardar"].forEach((btnId) => {
    ...
    function cambiarEstado(id, url, action) {
        mostrarConfirmacion(
            `¿Deseas ${action} el estado de reparación?`,
            () => {
                return new Promise((resolve, reject) => {
                    fetch(url, {
                        method: "POST",
